...
function CartItem({ items, totalPrice, setTotalPrice }) {
  ...
  var p1 = items.item.price.replace("Rs. ", "");
  var p2 = p1.replace("/-", "");
  return (
    <Container>
      ...
      <div className="details-container">
        <div style={{ display: 'flex' }}><div className="title">{items.item.name}</div> {/* <div onClick={} style={{ paddingLeft: "300px", color:"red", fontWeight:'bold' }}>X</div> */}</div>


        <div className="price-container">
          <div className="unit-price">
            <span>Unit Price: </span>{items.item.price}
          </div>
          <div className="total-price">
            <span>Total Price: </span>Rs.{p2 * items.qty}/-
          </div>
        </div>
      </div>
    </Container>
