...
function ItemSection() {
  ...
  const [name, setItemName] = useState(0);
  const [address, setItemAd] = useState(0);
  const [con, setItemCon] = useState(0);
      ...
      {itemClicked ? (
        <PopUpContainer>
          ...
          <div className="popup">
            ...
            {/*  <div className="input-field">
              <span>Total: </span>
              <p style={{color:"white"}} >{clickedItemDetails.total* clickedItemDetails.price}</p>
            </div> */}
      ...
      {cartClick ? (
        <CartContainer>
          ...
          <div className="cart">
            ...
            <div className="input-field">
              <span>Name: </span>
              <input type="text" onInput={(e) => setItemName(e.target.value)} name="quantity-needed" id="quantity-needed" max={clickedItemDetails.quantityAvailable} />
            </div><div className="input-field">
              <span>Contact: </span>
              <input type="text" onInput={(e) => setItemCon(e.target.value)} name="quantity-needed" id="quantity-needed" max={clickedItemDetails.quantityAvailable} />
            </div>
            <div className="input-field">
              <span>Deliver Address: </span>
              <input type="text" onInput={(e) => setItemAd(e.target.value)} name="quantity-needed" id="quantity-needed" max={clickedItemDetails.quantityAvailable} />
            </div>

            <div className="btn-container" style={{ height: '200px' }}>

              <div className="total-pay">
                <span style={{ color: 'black' }}>Total: </span> {cartArray.map((cartItem, index) => {
                  var p1 = cartItem.item['price'].replace("Rs. ", "");
                  var p2 = p1.replace("/-", "");
                  t += parseFloat(p2 * cartItem.qty);
                ...
                <div className="btn" onClick={() => {
                  if (cartArray.length > 0) {
                    cartArray.forEach((item, index) => {
                      ...
                      var p1 = item.item.price.replace("Rs. ", "");
                      var p2 = p1.replace("/-", "");
                      var total = item.qty * p2;
                      axios
                        .get("http://localhost:5000/order?name=" + item.item.name + "&qty=" + item.qty + "&total=" + total + "&ads=" + name + ", " + address + "&con=" + con)
                        .then((res) => {
                    ...
                    setCartArray([]);
                    setTotalPrice(0);
                    alert("Your Order will be deliver within three days !")
                  }
